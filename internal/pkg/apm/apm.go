...
func (s *APM) Shutdown(ctx context.Context) error {
	...
	return g.Wait() //nolint:wrapcheck //false positive
}
...
// GetTracerProvider is used to integrate otel with other client pkg (redis/kafka).
func (s *APM) GetTracerProvider() trace.TracerProvider { //nolint:ireturn,nolintlint
	if s.tracerProvider == nil {
...
// GetMeterProvider is used to integrate otel with other client pkg (redis/kafka).
func (s *APM) GetMeterProvider() metric.MeterProvider { //nolint:ireturn // that's how otel sdk works
	if s.meterProvider == nil {
...
func newMeter(opts *Options) (metric.MeterProvider, *Meter, error) { //nolint:ireturn // that's how otel sdk works
...
func newTracer(ctx context.Context, opts *Options) (trace.TracerProvider, *Tracer, error) { //nolint:ireturn // that's how otel sdk works
	var (
